...
def load(state_path: str or None, flow_path: str or None, firm: str):
    state_files = [file for file in listdir(state_path) if isfile(join(state_path, file))][:8]
    flow_files = [file for file in listdir(flow_path) if isfile(join(flow_path, file))][:8]
    ...
    traints_s.to_parquet(f'loaded_data/timeseries/{firm}_states.parquet.gzip', compression='gzip')

    trainoe_s.to_parquet(f'loaded_data/order_eval/{firm}_states.parquet.gzip', compression='gzip')
    trainoe_f.to_parquet(f'loaded_data/order_eval/{firm}_flow.parquet.gzip', compression='gzip')
...
def preprocess(statedf: pd.DataFrame, flowdf: pd.DataFrame, window: int, horizon: int, path: str):
    # Group by time and get the minimum 'STEP_CREATED' for each group
...
def make_batches(horizon: int, window: int, state_path: str or None, flow_path: str or None):
    ...
    for i, firm in enumerate(firms):
        ts_state_file = f'loaded_data/timeseries/{firm}_states.parquet.gzip'
        oe_flow_file = f'loaded_data/order_eval/{firm}_flow.parquet.gzip'
        oe_state_file = f'loaded_data/order_eval/{firm}_states.parquet.gzip'
        oe = f'preprocessed_data/{firm}merged_w{window}_h{horizon}.parquet.gzip'
        ts = f'preprocessed_data/{firm}ts_w{window}_h{horizon}.parquet.gzip'
        ...
        if os.path.exists(oe):
            ...
            merged = preprocess(oe_statedf, oe_flowdf, window, horizon, oe)
        ...
        if os.path.exists(ts):
            ...
            timeseries_df = get_ts(window=window, horizon=horizon, df=ts_statedf, path_=ts)
